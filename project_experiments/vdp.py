...
class Conv2d(torch.nn.Module):
    ...
    def forward(self, mu_x, sigma_x=torch.tensor(0., requires_grad=True)):
        if self.input_flag:
            ...
            a = softplus(self.sigma.weight).repeat(1, vec_x.shape[1])
            print(mu_x.shape, vec_x.shape, self.sigma.weight.shape, a.shape)
            sigma_y = a @ vec_x ** 2
...
class ConvTranspose2d(torch.nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=(5, 5), stride=1, 
                 ...
                 padding_mode='zeros', input_flag=False):
        ...
        self.unfold = torch.nn.Unfold(kernel_size, dilation, padding, stride)
        print("aa",self.mu.weight.shape, self.sigma.weight.view(1,-1,1,1).repeat(in_channels, 1, kernel_size[0], kernel_size[1]).shape)
...
def ELBOLoss(mu, sigma, y, num_classes=10, criterion='nll'):
    y_hot = F.one_hot(y, num_classes=num_classes)
